...
const PatrolForm = React.lazy(() => import('./patrol-add'));

const PatrolList = () => {
  const [show, setShow] = useState(false);
  const handleClose = () => setShow(false);
  const handleShow = () => setShow(true);
  const [patrolToEdit, setPatrolToEdit] = useState({
    _id: '',
    supervisor: { name: '', email: '', password: '', rank: '' },
    status: '',
    location: '',
    teamMembers: [],
    assignedMissions: []
  ...
  const refresh = () => {
    fetchData(`${globalConfig.BACKEND_URL}/api/patrols/`)
      .then(response => setPatrollist(response))
  }
            ...
            <Card>
              <Card.Header className="d-flex justify-content-between">
                ...
                <div>
                  ...
                  <PatrolForm refresh={refresh} show={show} handleClose={handleClose} handleShow={handleShow} patrolToEdit={patrolToEdit} mode={mode}> </PatrolForm>
                </div>
              </Card.Header>
                    ...
                    <tbody>
                      {patrollist.map((item, idx) => (
                        <tr key={idx}>
                          ...
                          <td>
                            <div style={statusStyles[item.status] || statusStyles.off_duty}></div>{' '}
                            {item.status}
                          </td>
                          <td>{item.location}</td>
                          ...
                          <td>
                            <div className="flex align-items-center list-patrol-action">
                              <Link onClick={() => {
                                setMode('edit');
                                setPatrolToEdit(item);
                                handleShow();
                                console.log(patrolToEdit);
                              }}
                              ...
                              <Link onClick={async () => {
                                confirmDelete(`${globalConfig.BACKEND_URL}/api/patrols/delete/${item._id}`, refresh)
